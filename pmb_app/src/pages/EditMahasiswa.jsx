...
function EditMahasiswa() {
  ...
  const fetchData = async () => {
    try {
      const response = await fetch(`http://localhost:5000/getData/${id}`);
  ...
  console.log("Image Data Array:", data[0]?.foto?.data);

  useEffect(() => {
    if (data.length > 0 && data[0]?.foto?.data) {
      try {
        // Convert the Uint8Array data to a binary string
        const binaryString = String.fromCharCode(
          ...new Uint8Array(data[0].foto.data)
        );

        // Convert the binary string to a base64 encoded string
        const base64String = btoa(binaryString);

        // Log the base64 string to verify the conversion
        console.log("Base64 String:", base64String);

        // Set the photo preview with the appropriate MIME type
  ...
  useEffect(() => {
    if (data.length > 0) {
      //   setProvinces(editData.province);
      //   setCities(editData.city);
      //   setDistricts(editData.district);
    }
  });
  ...
  const handleSubmit = (e) => {
    ...
    fetch(`http://localhost:5000/putData/${id}`, {
      method: "PUT",
      body: data,
    })
      .then((response) => response.json())
